...
def NYCBIKEDataLoader(dir='data/NYCBIKE20160708/', batch_size=64):
    x = np.load(dir+'x_flow.npy')
    y = np.load(dir+'y_flow.npy')
    x = x.reshape(x.shape[0], x.shape[1], x.shape[2]*x.shape[3], x.shape[4])
    y = y.reshape(y.shape[0], y.shape[1], y.shape[2]*y.shape[3], y.shape[4])
    data = {}
    # set of NYCBIKE20150102
    #data['train_loader'] = DataLoader(x[:976], y[:976], batch_size)
    #data['test_loader'] = DataLoader(x[976:976+139], y[976:976+139], batch_size)
    #data['val_loader'] = DataLoader(x[976+139:], y[976+139:], batch_size)
    # set of NYCBIKE201501-12 12To12
    # data['train_loader'] = DataLoader(x[:6343], y[:6343], batch_size)
    # data['test_loader'] = DataLoader(x[6343:6343+907], y[6343:6343+907], batch_size)
    # data['val_loader'] = DataLoader(x[6343+907:], y[6343+907:], batch_size)
    # set of NYCBIKE201501-12 6To1
    data['train_loader'] = DataLoader(x[:6355], y[:6355], batch_size)
    data['val_loader'] = DataLoader(x[6355:6355+908], y[6355:6355+908], batch_size)
    data['test_loader'] = DataLoader(x[6355+908:], y[6355+908:], batch_size)
    
    return data

def NYCBIKEGraphDataLoader(dir, batch_size=64):
    x = np.load(dir+'x_flow.npy')
    y = np.load(dir+'y_flow.npy')
    x = x.reshape(x.shape[0], x.shape[1], x.shape[2]*x.shape[3], x.shape[4])
    y = y.reshape(y.shape[0], y.shape[1], y.shape[2]*y.shape[3], y.shape[4])
...
def main():
    x = np.load('data/NYCBIKE201501-201512/x_flow.npy')
    print(x.shape)

    

def NYCBIKEODDataLoader(dir='data/NYCBIKE20160708/', batch_size=64):
    x = np.load(dir+'x_od.npy')
    y = np.load(dir+'y_od.npy')
    x = x.reshape(x.shape[0], x.shape[1], x.shape[2]*x.shape[3], x.shape[4])
    y = y.reshape(y.shape[0], y.shape[1], y.shape[2]*y.shape[3], y.shape[4])
    data = {}
    # set of NYCBIKE20150102
    #data['train_loader'] = DataLoader(x[:974], y[:974], batch_size)
    #data['test_loader'] = DataLoader(x[974:974+140], y[974:974+140], batch_size)
    #data['val_loader'] = DataLoader(x[974+140:], y[974+140:], batch_size)
    data['train_loader'] = DataLoader(x[:6107], y[:6107], batch_size)
    data['test_loader'] = DataLoader(x[6107:6107+872], y[6107:6107+872], batch_size)
    data['val_loader'] = DataLoader(x[6107+872:], y[6107+872:], batch_size)
    
    return data
    

def MetrLADataLoader(dir='data/METR-LA/', batch_size=64):
    train_dir = dir+'train.npz'
    test_dir = dir+'test.npz'
    val_dir = dir+'val.npz'
    train_data = np.load(train_dir)
    test_data = np.load(test_dir)
    val_data = np.load(val_dir)
    data = {}
    data['train_loader'] = DataLoader(train_data['x'], train_data['y'], batch_size)
    data['test_loader'] = DataLoader(test_data['x'], test_data['y'], batch_size)
    data['val_loader'] = DataLoader(val_data['x'], val_data['y'], batch_size)
